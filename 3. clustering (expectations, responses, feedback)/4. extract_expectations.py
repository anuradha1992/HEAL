...
with open('./all_expectations.csv', 'a', encoding='utf-8') as outfile:

  writer = csv.writer(outfile, delimiter=str(','), lineterminator='\n')        
  new_row = ['cluster_id', 'dialog_id', 'question']
    ...
    with open('./clustered_dialogues/'+str(i)+'.csv', 'r', encoding='utf-8') as infile:
      ...
      for row in readCSV:
        ...
        try:
          ...
          if turn == 1 and 'speaker' in actor:
...
with open('question_counts.txt', 'w') as file:
...
fig.update_layout(
  height=450,
  width=900,
  title_text='Distribution of speaker expectations per each cluster',
  xaxis_tickangle=-90,
  font=dict(
...
print("Total no. of expectations: ", tot_responses)
print("Average no. of expectations per cluster: ", round(tot_responses/tot_clusters,2))
print("Max no. of expectations per cluster:", max(list(response_dict.values())))
print("Min no. of expectations per cluster:", min(list(response_dict.values())))
