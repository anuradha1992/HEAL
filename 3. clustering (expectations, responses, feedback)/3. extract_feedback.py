...
with open('./all_feedback.csv', 'a', encoding='utf-8') as outfile:

  writer = csv.writer(outfile, delimiter=str(','), lineterminator='\n')        
  new_row = ['cluster_id', 'dialog_id', 'feedback']
    ...
    with open('./clustered_dialogues/'+str(i)+'.csv', 'r', encoding='utf-8') as infile:
      ...
      for row in readCSV:
        ...
        try:
          ...
          if turn == 3 and 'speaker' in actor and 'listener' in prev_row[4]:
...
with open('feedback_counts.txt', 'w') as file:
...
fig.update_layout(
  height=450,
  width=900,
  title_text='Distribution of speaker feedback per each cluster',
  xaxis_tickangle=-90,
  font=dict(
...
print("Total no. of feedback: ", tot_responses)
print("Average no. of feedback per cluster: ", round(tot_responses/tot_clusters,2))
print("Max no. of feedback per cluster:", max(list(response_dict.values())))
print("Min no. of feedback per cluster:", min(list(response_dict.values())))
...
'''
Total no. of feedback:  20213
Average no. of feedback per cluster:  4.63
Max no. of feedback per cluster: 5040
Min no. of feedback per cluster: 0feef
'''
